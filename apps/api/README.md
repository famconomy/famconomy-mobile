...
2.  **Navigate to the API workspace:**
    ```bash
    cd apps/api
    ```

3.  **Create a `.env` file:**
    Create a `.env` file in the root of the `apps/api` directory and add the following environment variables:
...
4. Start the backend (`npm run api:dev`) and frontend (`npm run web:dev`), visit the login page, and use the corresponding provider button.
